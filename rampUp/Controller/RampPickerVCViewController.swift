...
class RampPickerVCViewController: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        if let node = obj?.rootNode.childNode(withName: "pipe", recursively: true) {
        ...
        if let node = obj?.rootNode.childNode(withName: "pyramid", recursively: true) {
            node.runAction(rotate)
        ...
        if let node = obj?.rootNode.childNode(withName: "quarter", recursively: true) {
            node.runAction(rotate)
            node.scale = SCNVector3Make(0.0058, 0.0058, 0.0058)
